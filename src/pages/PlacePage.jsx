    ...
    <div className="mt-4 -mx-8 px-8 pt-8">
      ...
      <div>
        <div className="flex justify-between items-center">  
          ...
          <button className="flex gap-2 items-center cursor-pointer transition bg-white ease-in-out delay-100 -translate-y-1 hover:scale-110 hover:bg-gray-100 duration-300 p-2 rounded-xl" onClick={copyLink}>
      ...
      <div className="mt-8 mb-8 gap-8 grid grid-cols-1 md:grid-cols-[2fr_1fr]">
        <div>
          <div className="my-4">
            ...
            {displayText + " "}
            {text.length > 600 && (
              <button className="bg-white font-semibold underline" onClick={toggleExpanded}>
          ...
          <b className="underline">Maximum number of guests: </b>  {place.maxGuests} <br />
          <h3 className="text-2xl mt-4 font-semibold">What this place offers </h3>
          <div className="grid grid-cols-2">
            {place.perks.map(perk => (
              <div className="flex gap-2 text-lg" key={perk}>
                ...
                <p>{perk.substring(0, 1).toUpperCase() + perk.substring(1, perk.length)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mx-8 px-8 py-8 border-t">
        ...
        <div className="mb-4 mt-2 text-md text-gray-600 leading-5">{place.extraInfo}</div>
      </div>
    </div>
  )
}
